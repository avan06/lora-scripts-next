Schema.intersect([
    ...
    Schema.object(
        UpdateSchema(SHARED_SCHEMAS.RAW.PRECISION_CACHE_BATCH, {
            ...
            cache_latents: Schema.boolean().default(true).description("缓存图像 latent，减少训练时 VAE 重算。图像编辑模式会自动开启，用于缓存 Target 与 Reference latent"),
            cache_latents_to_disk: Schema.boolean().default(false).description("缓存图像 latent 到磁盘。关闭时只在本次训练内存中缓存，不会写入数据集旁边的缓存文件"),
            cache_text_encoder_outputs: Schema.boolean().default(true).description("缓存文本编码器的输出，减少显存使用。图像编辑模式会自动开启；使用时需要关闭 shuffle_caption"),
            cache_text_encoder_outputs_to_disk: Schema.boolean().default(false).description("缓存文本编码器输出到磁盘。关闭时只在本次训练内存中缓存"),
